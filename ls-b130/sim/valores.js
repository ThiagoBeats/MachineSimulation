// ---------------------------------------------------------------------------
// CENARIO INICIAL DA MAQUINA LS-B130
// ---------------------------------------------------------------------------
// Funciona em Node (servidor local de desenvolvimento) e no navegador (site
// publicado). Informe so os campos que interessam: o que faltar e preenchido
// com o valor neutro do tipo correto.
//
// O que o aluno alterar pela tela fica no localStorage do navegador dele e tem
// precedencia sobre este arquivo. Abrir a simulacao com ?reset=1 volta a aqui.
// ---------------------------------------------------------------------------
...
// --- Produtos liquidos (linhas L1..L6) -------------------------------------
const LIQUIDOS = [
...
// controle (ignore-invalid-values), e o campo aparece zerado na tela.
// Cada linha de liquido/po tem: Step (ordem de injecao, 0 = nao usada),
// Dosage (mL ou g por 100 kg), InjectionTime (s), InjectionDelay (s), ProductID.
...
const RECEITAS = [
  {
    ...
    liquidos: [linha(1, 250.0, 8.0, 0.5, 1), linha(2, 120.0, 6.0, 0.5, 2), linha(3, 80.0, 5.0, 0.5, 5)],
  ...
  },
  {
    ...
    liquidos: [linha(1, 300.0, 9.0, 0.5, 1), linha(2, 200.0, 7.0, 0.5, 3), linha(3, 150.0, 6.0, 0.5, 5)],
  ...
  },
  {
    ...
    liquidos: [linha(1, 180.0, 7.0, 0.5, 4), linha(2, 90.0, 5.0, 0.5, 6)],
    pos: [linha(1, 350.0, 6.0, 0.5, 2)]
  },
  {
    RecipeName: 'FEIJAO TRATADO', BatchWeight: 400.0,
    MixerSpeed: 70.0,  MixingTime: 40, UnloadSpeed: 60.0,  UnloadTime: 25,
    liquidos: [linha(1, 220.0, 8.0, 0.5, 2), linha(2, 100.0, 5.0, 0.5, 5)],
    pos: [linha(1, 300.0, 6.0, 0.5, 1)]
  },
  {
    RecipeName: 'ALGODAO ESPECIAL', BatchWeight: 850.0,
    MixerSpeed: 110.0, MixingTime: 70, UnloadSpeed: 85.0,  UnloadTime: 40,
    liquidos: [linha(1, 350.0, 10.0, 0.5, 3), linha(2, 260.0, 8.0, 0.5, 1), linha(3, 120.0, 6.0, 0.5, 5)],
...
// O CLP declara os arrays com limite inferior 1, entao o slot JSON 0 equivale
// ao indice 1 do CLP - que e o primeiro slot util.
const N_SLOTS = 100;
...
function montarReceita(r, slotJson) {
  ...
  // TotalTime conforme a conta do CLP: pico de (atraso + tempo) por etapa,
  // somado, mais tempo de mistura e de descarga.
  let total = 0;
  ...
  return {
    ...
    MixerSpeed: r.MixerSpeed,
    MixingTime: r.MixingTime,
    UnloadSpeed: r.UnloadSpeed,
    UnloadTime: r.UnloadTime,
    TotalTime: total,
    LiquidsConfig: liquidos,
    PowderConfig: pos
  };
}
...
// --- Consumo ---------------------------------------------------------------
// GoalDosis e a dose ALVO de cada linha (ARRAY[1..25] OF REAL) e LastDosis o
// historico realizado (20 dosagens x 15 linhas). As realizadas variam em torno
// da alvo, para o popup de consumo parecer um historico de verdade.
const N_LINHAS = LIQUIDOS.length + POS.length;
function doseAlvo(linha) { return 250 - linha * 28; }
...
for (let dose = 0; dose < 20; dose++) {
  ...
  for (let l = 0; l < 15; l++) {
    if (l < N_LINHAS) {
      const base = doseAlvo(l);
      const variacao = ((dose * 7 + l * 13) % 11) - 5;
      linhaDose.push(Math.round((base + variacao) * 10) / 10);
    } else {
      linhaDose.push(0);
    }
  }
  ...
  // Modo de operacao (o botao Manual/Automatico da tela escreve aqui)
  ...
  'PLC1.Batches.LoadedBatch.BatchName': 'OP-2026-0413',
  'PLC1.Batches.LoadedBatch.BatchWeight': 12500.0,
  'PLC1.Batches.ProcessedWeight': 4820.5
  };
});
